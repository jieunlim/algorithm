
# 55. Jump Game

# Jump+Game.docx

class Solution(object):

    def canJump2(self, nums):
        def canJumpRe(pos):
            print(f"pos={pos}, memo={memo}")
            if pos == len(nums)-1:
                return True

            if pos in memo:
                print(f"memo return {pos}, {memo[pos]}")
                return memo[pos]

            maxStep = min(len(nums)-1, pos+nums[pos])
            print(f" pos={pos},  maxStep={maxStep}")
            for i in range(pos, maxStep):
                print(f"i={i}")
                if canJumpRe(i+1): return True

            memo[pos] = False
            return memo[pos]
        memo = {}
        return canJumpRe(0)
    # O(n)
    def canJump(self, nums):
        maxIdx = 0
        for i in range(len(nums)):
            print(f"i={i}, nums[i]={nums[i]}, max={maxIdx}")
            if i > maxIdx:
                return False
            print(f"i + nums[i]={i + nums[i]}")
            if i + nums[i] > maxIdx:
                maxIdx = i + nums[i]

        return True
nums=[2,3,1,1,4]
# nums=[2,1,0,4]
# nums=[2,0,6,9,8,4,5,0,8,9,1,2,9,6,8,8,0,6,3,1,2,2,1,2,6,5,3,1,2,2,6,4,2,4,3,0,0,0,3,8,2,4,0,1,2,0,1,4,6,5,8,0,7,9,3,4,6,6,5,8,9,3,4,3,7,0,4,9,0,9,8,4,3,0,7,7,1,9,1,9,4,9,0,1,9,5,7,7,1,5,8,2,8,2,6,8,2,2,7,5,1,7,9,6]

obj = Solution()
print(obj.canJump(nums))

# 1306. Jump Game III